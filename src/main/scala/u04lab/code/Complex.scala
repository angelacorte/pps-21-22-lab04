...
case class ComplexImpl(_re: Double, _im: Double) extends Complex:

  override def re: Double = _re

  override def im: Double = _im

  override def +(c: Complex): Complex = Complex(_re + c.re, _im + c.im)

  override def *(c: Complex): Complex = Complex(_re * c.re - _im * c.im, _re * c.im + c.re * _im)